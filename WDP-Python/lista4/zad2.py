...
def parzysta(n):
    return bool(n%2 - 1) # Ups, wszystkie liczby sa parzyste [!] edit
...
def histogram(L):
    ...
    for liczba in L:
        print (ze_spacjami(liczba,3) + "*" * liczba) #[!] edit
...
def powieksz_liczby(L):
    ...
    for i in range(len(L)):
        L[i] += 1 #[!] (pass oznacza "nic nie rob") edit
...
def normalizuj(L):
    ...
    for liczba in L:
        srednia += liczba
    srednia = srednia / len(L) # chyba jednak nie zawsze, [!] edit
...
def powiekszone_parzyste_z_zerami(L): #[!] drobny błąd edit
