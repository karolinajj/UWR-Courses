...
def most_popular(index, word):
    ...
    threads = []
    for url in pages: #new thread for each url that counts how many times the given world appeard on each page
        thread = threading.Thread(target=lambda: word_count.append((count_word(url, word), url)))
